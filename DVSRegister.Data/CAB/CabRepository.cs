...
namespace DVSRegister.Data.CAB
{
    public class CabRepository : ICabRepository
    {
        ...
        {
            return await context.Role.Include(x => x.TrustFrameworkVersion)
                .Where(x => x.TrustFrameworkVersion.Version <= tfVersion).OrderBy(c => c.Order).ToListAsync();
        }
        ...
        public async Task<bool> CheckProviderRegisteredNameExists(string registeredName)
        {
            var existingProvider =
                await context.ProviderProfile.FirstOrDefaultAsync(p =>
                    p.RegisteredName.ToLower() == registeredName.ToLower());
        ...
        {
            var existingProvider = await context.ProviderProfile.FirstOrDefaultAsync(p =>
                p.RegisteredName.ToLower() == registeredName.ToLower() && p.Id != providerId);
        ...
        public async Task<List<ProviderProfile>> GetProviders(int cabId, string searchText = "")
        {
            ...
            IQueryable<ProviderProfile> providerQuery = context.ProviderProfile
                .Include(p => p.Services!.Where(s => s.CabUser.CabId == cabId)).ThenInclude(s => s.CabUser)
        ...
        public async Task<ProviderProfile> GetProvider(int providerId, int cabId)
        {
            ...
            provider = await context.ProviderProfile.Include(p => p.Services)!.ThenInclude(p => p.CertificateReview)
                           ...
                           .Include(p => p.Services!.Where(s => s.CabUser.CabId == cabId))
                           .ThenInclude(p => p.CabTransferRequest)!
                           .ThenInclude(p => p.RequestManagement)
                           .Include(p => p.ProviderProfileCabMapping).ThenInclude(cu => cu.Cab)
                           .Where(p => p.Id == providerId && p.ProviderProfileCabMapping.Any(m => m.CabId == cabId))
                           .OrderBy(p => p.ModifiedTime != null ? p.ModifiedTime : p.CreatedTime)
                           .FirstOrDefaultAsync() ??
                       new ProviderProfile();
            return provider;
        ...
        {
            var service = await context.Service.AsNoTracking()
        ...
        public async Task<Service> GetServiceDetails(int serviceId, int cabId)
        {
            ...
            if (await baseQuery.AnyAsync(p =>
                    p.ServiceQualityLevelMapping != null && p.ServiceQualityLevelMapping.Any()))
            {
            ...
            if (await baseQuery.AnyAsync(p =>
                    p.ServiceIdentityProfileMapping != null && p.ServiceIdentityProfileMapping.Any()))
            {
        ...
        {
            ProviderProfile provider = new();
            provider = await context.ProviderProfile.Include(p => p.Services).Include(p => p.ProviderProfileCabMapping)
                .ThenInclude(p => p.Cab).Where(x => x.Id == providerId).FirstOrDefaultAsync() ?? new ProviderProfile();
        ...
        {
            var pendingRequests = await context.RequestManagement.Where(r =>
                r.CabId == cabId && r.RequestStatus == RequestStatusEnum.Pending &&
                r.RequestType == RequestTypeEnum.CabTransfer).ToListAsync();

            var pendingUploads = await context.CabTransferRequest.Include(c => c.Service).ThenInclude(c => c.Provider)
                .Where(c => c.ToCabId == cabId && c.CertificateUploaded == false && c.RequestManagement != null &&
                            c.RequestManagement.RequestStatus == RequestStatusEnum.Approved).ToListAsync();
        ...
        {
            List<int> cabIds = await context.Service.Include(p => p.CabUser).Where(x => serviceIds.Contains(x.Id))
                .Select(x => x.CabUser.CabId).Distinct().ToListAsync();
            List<string> activeCabUserEmails = await context.CabUser
                .Where(c => cabIds.Contains(c.CabId) && c.AccountStatus == AccountStatusEnum.Active)
                .Select(c => c.CabEmail).ToListAsync();
        ...
        public async Task<ProviderProfile> GetProviderWithLatestVersionServices(int providerId, int cabId)
        {
            ...
            provider = await context.ProviderProfile
                           .Include(p =>
                               p.Services!.Where(s => s.CabUser.CabId == cabId && latestServiceIds.Contains(s.Id)))
                           .ThenInclude(s => s.CertificateReview)
                           .Include(p =>
                               p.Services!.Where(s => s.CabUser.CabId == cabId && latestServiceIds.Contains(s.Id)))
                           .ThenInclude(s => s.PublicInterestCheck)
                           .Include(p =>
                               p.Services!.Where(s => s.CabUser.CabId == cabId && latestServiceIds.Contains(s.Id)))
                           .ThenInclude(p => p.ServiceDraft)
                           .Include(p =>
                               p.Services!.Where(s => s.CabUser.CabId == cabId && latestServiceIds.Contains(s.Id)))
                           .ThenInclude(s => s.CabUser)
            ...
            string loggedInUserEmail)
        {
            ...
            try
            {
                var existingProvider = await context.ProviderProfile.Where(p => p.Id == providerProfile.Id)
                    .FirstOrDefaultAsync();
                if (existingProvider != null)
                {
        ...
        public async Task<GenericResponse> SaveService(Service service, string loggedInUserEmail)
        {
            ...
            try
            {
                var existingService = await context.Service.Include(x => x.ServiceRoleMapping)
                    .Include(x => x.ServiceIdentityProfileMapping)
                    .Include(x => x.ServiceQualityLevelMapping).Include(x => x.ServiceSupSchemeMapping)
                    .Include(x => x.ManualUnderPinningService)
                    .Where(x => x.ServiceKey > 0 && x.ServiceKey == service.ServiceKey && service.IsCurrent == true)
                    .FirstOrDefaultAsync();
                if (existingService != null && existingService.Id > 0)
                {
                    ...
                    if (service.ServiceStatus == ServiceStatusEnum.SavedAsDraft)
                        await context.SaveChangesAsync(TeamEnum.CAB, EventTypeEnum.SaveAsDraftService,
                            loggedInUserEmail);
                    else
                ...
                else
                {
                    ...
                    service.ConformityIssueDate = service.ConformityIssueDate == DateTime.MinValue
                        ? null
                    ...
                    genericResponse.InstanceId = entity.Entity.Id;
                    service.ServiceKey =
                        entity.Entity.Id; // for new service addition , assign service key same as that of primary key
            ...
            bool isReupload, InProgressApplicationParameters? inProgressApplicationParameters)
        {
            ...
            try
            {
                var existingService = await context.Service.Include(x => x.ServiceRoleMapping)
                    .Include(x => x.ServiceIdentityProfileMapping)!
                    .Include(x => x.ServiceQualityLevelMapping).Include(x => x.ServiceSupSchemeMapping)!
                    .ThenInclude(x => x.SchemeGPG44Mapping)
                ...
                if (existingService != null && existingService.Id > 0 && existingService.ServiceKey > 0)
                {
                    if (existingService.ServiceStatus == ServiceStatusEnum.SavedAsDraft) //Resuming save
                    {
                        ...
                        if (inProgressApplicationParameters != null)
                            await RemoveInprogressApplications(loggedInUserEmail, inProgressApplicationParameters,
                    ...
                    {
                        if (!isReupload && existingService.ServiceStatus != ServiceStatusEnum.Published &&
                        ...
                        {
                            var cabTransferRequest = context.CabTransferRequest.Include(c => c.RequestManagement)
                                .Where(c => c.ServiceId == existingService.Id).OrderByDescending(c => c.Id)
                                .FirstOrDefault();
                            ...
                            if (cabTransferRequest != null && cabTransferRequest.RequestManagement != null
                                                           && cabTransferRequest.RequestManagement.RequestType ==
                                                           RequestTypeEnum.CabTransfer
                                                           && cabTransferRequest.RequestManagement.RequestStatus ==
                                                           RequestStatusEnum.Approved
                                                           && cabTransferRequest.CertificateUploaded == false)
                            {
                                cabTransferRequest.CertificateUploaded = true;
                                if (existingService.ServiceStatus == ServiceStatusEnum.PublishedUnderReassign ||
                                    existingService.ServiceStatus == ServiceStatusEnum.RemovedUnderReassign)
                        ...
                        var serviceList = await context.Service.Where(x => x.ServiceKey == service.ServiceKey)
                            .ToListAsync();
                        ...
                        if (inProgressApplicationParameters?.HasInProgressApplication != true ||
                            service.ServiceStatus == ServiceStatusEnum.SavedAsDraft)
                        {
        ...
        public async Task<GenericResponse> SaveServiceAmendments(Service service, string loggedInUserEmail)
        {
            ...
            try
            {
                var existingService = await context.Service.Include(x => x.CertificateReview)
                    .Include(x => x.ServiceRoleMapping).Include(x => x.ServiceIdentityProfileMapping)
                    .Include(x => x.ServiceQualityLevelMapping).Include(x => x.ServiceSupSchemeMapping)
                    .Include(x => x.ManualUnderPinningService)
                ...
                if (existingService != null && existingService.Id > 0 && existingService.ServiceKey > 0)
                {
                    if (existingService.ServiceStatus == ServiceStatusEnum.AmendmentsRequired)
                    {
                        ...
                        await context.SaveChangesAsync(TeamEnum.CAB, EventTypeEnum.ServiceAmendments,
                            loggedInUserEmail);
                        genericResponse.InstanceId = existingService.Id;
                        await transaction.CommitAsync();
                        genericResponse.Success = true;
        ...
        private async Task RemoveInprogressApplications(string loggedInUserEmail,
            InProgressApplicationParameters? inProgressApplicationParameters, Service? existingService,
            Service? newService = null)
        {
            if (inProgressApplicationParameters == null) return;

            if ((inProgressApplicationParameters.HasInProgressApplication &&
                 inProgressApplicationParameters.InProgressApplicationId == existingService?.Id)
                || (inProgressApplicationParameters.InProgressAndUpdateRequested &&
                    inProgressApplicationParameters.InProgressAndUpdateRequestedId == existingService?.Id))
            {
                context.Remove(existingService);
                await context.SaveChangesAsync(TeamEnum.CAB, EventTypeEnum.RemoveInProgressApplication,
                    loggedInUserEmail);
            }


            else if (inProgressApplicationParameters.HasInProgressApplication && inProgressApplicationParameters
                                                                                  .InProgressApplicationId !=
                                                                              existingService?.Id
                                                                              && newService?.ServiceStatus !=
                                                                              ServiceStatusEnum.SavedAsDraft &&
                                                                              newService != null)
            {
                var inprogressServiceBeforeDraft = await context.Service.Include(x => x.ServiceRoleMapping)
                    .Include(x => x.ServiceIdentityProfileMapping)!
                    .Include(x => x.ServiceQualityLevelMapping).Include(x => x.ServiceSupSchemeMapping)!
                    .ThenInclude(x => x.SchemeGPG44Mapping)
                    .Include(x => x.ServiceSupSchemeMapping)!.ThenInclude(x => x.SchemeGPG45Mapping)
                ...
                if (inprogressServiceBeforeDraft != null)
                {
                    ...
                    existingService.ServiceVersion =
                        existingService.ServiceVersion - 1; // as previous service is removed, decrease version by 1
                    await context.SaveChangesAsync(TeamEnum.CAB, EventTypeEnum.RemoveInProgressApplication,
                        loggedInUserEmail);
                }
            }

            else if (inProgressApplicationParameters.InProgressAndUpdateRequested &&
                     inProgressApplicationParameters.InProgressAndUpdateRequestedId != existingService?.Id
                     && newService?.ServiceStatus != ServiceStatusEnum.SavedAsDraft && newService != null)
            {
                var inprogressServiceBeforeDraft = await context.Service.Include(x => x.ServiceRoleMapping)
                    ...
                    .Include(s => s.ActionLogs)
                    .Include(s => s.ProceedApplicationConsentToken)
                    .Where(x => x.Id == inProgressApplicationParameters.InProgressAndUpdateRequestedId)
                    .FirstOrDefaultAsync();
            ...
            if (inProgressApplicationParameters.HasActiveRemovalRequest &&
                inProgressApplicationParameters.InProgressRemovalRequestServiceId > 0)
            {
                var removalService = await context.Service
                    .Include(s => s.ServiceRemovalRequest)!
                    .Where(x => x.Id == inProgressApplicationParameters.InProgressRemovalRequestServiceId)
                    .FirstOrDefaultAsync();

                var serviceRemovalRequest = removalService?.ServiceRemovalRequest
                    ?.Where(x => x.IsRequestPending == true).FirstOrDefault();
            ...
            if (inProgressApplicationParameters.HasActiveReassignmentRequest &&
                inProgressApplicationParameters.InProgressReassignmentRequestServiceId > 0)
            {
                var reassignmentService = await context.Service
                    .Include(s => s.CabTransferRequest)!.ThenInclude(tr => tr.RequestManagement)
                    .Where(x => x.Id == inProgressApplicationParameters.InProgressReassignmentRequestServiceId)
                    .FirstOrDefaultAsync();

                if (reassignmentService != null)
                {
                    var transferRequest = reassignmentService.CabTransferRequest!.OrderByDescending(c => c.Id)
                        .FirstOrDefault();
            ...
            if (inProgressApplicationParameters.HasActiveUpdateRequest &&
                inProgressApplicationParameters.InProgressUpdateRequestServiceIds != null &&
                inProgressApplicationParameters.InProgressUpdateRequestServiceIds.Count > 0)
            {
                foreach (var serviceId in inProgressApplicationParameters.InProgressUpdateRequestServiceIds)
                {
                    if (serviceId !=
                        inProgressApplicationParameters
                            .InProgressApplicationId) // if not already removed in first condition
                    {
                        ...
                        {
                            updateRequestService.ServiceStatus =
                                updateRequestService.ServiceDraft.PreviousServiceStatus;
        ...
        private void UpdateExistingServiceRecord(Service service, Service? existingService)
        {
            ...
            if (existingService.ServiceIdentityProfileMapping != null &
                existingService.ServiceIdentityProfileMapping?.Count > 0)
            ...
            if (existingService.ServiceQualityLevelMapping != null &
                existingService.ServiceQualityLevelMapping?.Count > 0)
            ...
            if (service.ServiceType == ServiceTypeEnum.WhiteLabelled)
            {
                ...
                if (service.IsUnderPinningServicePublished == true) // publised underpinning service selected
                {
                    ...
                    {
                        if (context.Service.Where(s =>
                                s.ManualUnderPinningServiceId == existingService.ManualUnderPinningServiceId).Count() >
                            1)
                        {
                ...
                if (service.IsUnderPinningServicePublished == false)
                {
                    ...
                    {
                        existingService.ManualUnderPinningService =
                            service.ManualUnderPinningService; // insert as new manaul under pinning service
                    }

                    else if (service.ManualUnderPinningServiceId != null ||
                             service.ManualUnderPinningServiceId != 0) // a manual under pinning service updated
                    {
                        if (service.ManualUnderPinningServiceId != existingService.ManualUnderPinningServiceId)
                        {
                            if (context.Service.Where(s =>
                                        s.ManualUnderPinningServiceId == existingService.ManualUnderPinningServiceId)
                                    .Count() <= 1)
                            {
                                var manualServiceToRemove = context.ManualUnderPinningService
                                    .FirstOrDefault(s => s.Id == existingService.ManualUnderPinningServiceId);
                        ...
                        if (existingService.ManualUnderPinningService != null &&
                            service.ManualUnderPinningService != null)
                            // if there is an already existing manual service mapping update it
                        {
                            existingService.ManualUnderPinningService.ServiceName =
                                service.ManualUnderPinningService.ServiceName;
                            existingService.ManualUnderPinningService.ProviderName =
                            ...
                            existingService.ManualUnderPinningService.CertificateExpiryDate =
                                service.ManualUnderPinningService.CertificateExpiryDate;
                        }
                    }
                }
            }
            else
            {
                ...
                {
                    if (context.Service.Where(s =>
                            s.ManualUnderPinningServiceId == existingService.ManualUnderPinningServiceId).Count() > 1)
                    {
                        existingService.ManualUnderPinningServiceId = null;
                    }
                    else
        ...
        private void UpdateExistingProfileRecord(ProviderProfile existingProfile, ProviderProfile newProfile)
        {
            ...
            existingProfile.HasRegistrationNumber =
                newProfile.HasRegistrationNumber ?? existingProfile.HasRegistrationNumber;
            if (newProfile.HasRegistrationNumber == true)
            {
                existingProfile.CompanyRegistrationNumber = newProfile.CompanyRegistrationNumber ??
                                                            existingProfile.CompanyRegistrationNumber;
            ...
            existingProfile.ParentCompanyRegisteredName = newProfile.ParentCompanyRegisteredName ??
                                                          existingProfile.ParentCompanyRegisteredName;
            existingProfile.ParentCompanyLocation =
                newProfile.ParentCompanyLocation ?? existingProfile.ParentCompanyLocation;
            existingProfile.PrimaryContactFullName =
                newProfile.PrimaryContactFullName ?? existingProfile.PrimaryContactFullName;
            existingProfile.PrimaryContactJobTitle =
                newProfile.PrimaryContactJobTitle ?? existingProfile.PrimaryContactJobTitle;
            existingProfile.PrimaryContactEmail = newProfile.PrimaryContactEmail ?? existingProfile.PrimaryContactEmail;
            existingProfile.PrimaryContactTelephoneNumber = newProfile.PrimaryContactTelephoneNumber ??
                                                            existingProfile.PrimaryContactTelephoneNumber;
            existingProfile.SecondaryContactFullName =
                newProfile.SecondaryContactFullName ?? existingProfile.SecondaryContactFullName;
            existingProfile.SecondaryContactJobTitle =
                newProfile.SecondaryContactJobTitle ?? existingProfile.SecondaryContactJobTitle;
            existingProfile.SecondaryContactEmail =
                newProfile.SecondaryContactEmail ?? existingProfile.SecondaryContactEmail;
            existingProfile.SecondaryContactTelephoneNumber = newProfile.SecondaryContactTelephoneNumber ??
                                                              existingProfile.SecondaryContactTelephoneNumber;
            existingProfile.PublicContactEmail = newProfile.PublicContactEmail ?? existingProfile.PublicContactEmail;
            existingProfile.ProviderTelephoneNumber =
                newProfile.ProviderTelephoneNumber ?? existingProfile.ProviderTelephoneNumber;
            existingProfile.ProviderWebsiteAddress =
                newProfile.ProviderWebsiteAddress ?? existingProfile.ProviderWebsiteAddress;
