...
namespace DVSRegister.Data
{
    public class RegisterRepository : IRegisterRepository
    {
        ...
        public async Task<PaginatedResult<ProviderProfile>> GetProviders(List<int> roles, List<int> schemes,
            List<int> tfVersions, int pageNum, string searchText = "", string sortBy = "")
        {
            ...
            var providerQuery = context.ProviderProfile.Where(p => p.IsInRegister &&
                                                                   p.Services!.Any(ci => ci.IsInRegister &&
                                                                       (!hasSearch
                                                                        || ci.ServiceName!.ToLower()
                                                                            .Contains(lowerSearchText)
                                                                        || ci.Provider.RegisteredName.ToLower()
                                                                            .Contains(lowerSearchText)
                                                                        || ci.Provider.TradingName.ToLower()
                                                                            .Contains(lowerSearchText)
                                                                        || ci.TrustmarkNumber.TrustMarkNumber.Contains(
                                                                            lowerSearchText)) &&
                                                                       (!hasRoles || ci.ServiceRoleMapping!.Any(r =>
                                                                           roles.Contains(r.RoleId))) &&
                                                                       (!hasSchemes ||
                                                                        ci.ServiceSupSchemeMapping!.Any(s =>
                                                                            schemes.Contains(s.SupplementarySchemeId) &&
                                                                            s.ServiceSupSchemeCustomDisplayId ==
                                                                            null)) &&
                                                                       (!hasTfVersions ||
                                                                        tfVersions.Contains(
                                                                            ci.TrustFrameworkVersionId))))
                .Include(p => p.Services!
                    .Where(ci => ci.IsInRegister &&
                                 ...
                                 (!hasSchemes || ci.ServiceSupSchemeMapping!.Any(s =>
                                     schemes.Contains(s.SupplementarySchemeId) &&
                                     s.ServiceSupSchemeCustomDisplayId == null)) &&
                ...
                .Include(p => p.Services!).ThenInclude(ci =>
                    ci.ServiceSupSchemeMapping!.Where(s => s.ServiceSupSchemeCustomDisplayId == null))
                .AsSplitQuery();
        ...
        public async Task<PaginatedResult<Service>> GetServices(List<int> roles, List<int> schemes,
            List<int> tfVersions, int pageNum, string searchText = "", string sortBy = "")
        {
            ...
            var query = context.Service
                ...
                .Where(s => !schemes.Any() || s.ServiceSupSchemeMapping.Any(sc =>
                    schemes.Contains(sc.SupplementarySchemeId) && sc.ServiceSupSchemeCustomDisplayId == null))
        ...
        {
            var query = context.ActionLogs.Include(x => x.ActionDetails).Include(x => x.ProviderProfile)
                .ThenInclude(x => x.Services).Include(x => x.Service)
                .Where(x => x.ShowInRegisterUpdates == true && x.ProviderProfile.IsInRegister == true &&
                            (x.Service == null || x.ProviderProfile.Services.Any(s =>
                                s.ServiceKey == x.Service.ServiceKey && s.IsInRegister)));
        ...
        public async Task<DateTime?> GetLastUpdatedDate()
        {
            ...
            {
                latestUpdateDate = await context.ProviderProfile.Where(p => p.IsInRegister == true)
                    .MaxAsync(p => p.PublishedTime);
            }
        ...
        {
            var latestUpdateDate = await context.ActionLogs
                .Where(x => x.ShowInRegisterUpdates && x.ProviderProfileId == providerId)
                .MaxAsync(x => (DateTime?)x.LoggedTime);
            if (latestUpdateDate != null)
            {
                return latestUpdateDate;
            }

            return await context.ProviderProfile.Where(p => p.Id == providerId).Select(p => p.PublishedTime)
                .FirstOrDefaultAsync();
        }
        ...
        {
            var latestUpdateDate = await context.ActionLogs
                .Where(x => x.ShowInRegisterUpdates && x.ServiceId == serviceId).MaxAsync(x => (DateTime?)x.LoggedTime);
            if (latestUpdateDate != null)
            {
                return latestUpdateDate;
            }

            return await context.Service.Where(p => p.Id == serviceId).Select(p => p.PublishedTime)
                .FirstOrDefaultAsync();
        }
        ...
        public async Task<ProviderProfile> GetProviderDetails(int providerId)
        {
            ...
            providerProfile = await context.ProviderProfile
                                  ...
                                  .Include(p => p.Services!).ThenInclude(s => s.UnderPinningService)
                                  ...
                                  .ThenInclude(s => s.Cab)
                                  .Include(p => p.Services!).ThenInclude(s => s.ServiceSupSchemeMapping!)
                                  .ThenInclude(s => s.SupplementaryScheme)
                                  .Include(p => p.Services!).ThenInclude(s => s.ServiceSupSchemeMapping!)
                                  .ThenInclude(s => s.SchemeGPG44Mapping!).ThenInclude(s => s.QualityLevel)
                                  .Include(p => p.Services!).ThenInclude(s => s.ServiceSupSchemeMapping!)
                                  .ThenInclude(s => s.SchemeGPG45Mapping!).ThenInclude(s => s.IdentityProfile)
                                  ...
                                  .Include(p => p.Services!).ThenInclude(x => x.ServiceQualityLevelMapping!)
                                  .ThenInclude(p => p.QualityLevel)
                                  .Include(p => p.Services!).ThenInclude(p => p.TrustFrameworkVersion)
                                  .OrderBy(c => c.ModifiedTime)
                                  .FirstOrDefaultAsync(p => p.Id == providerId && p.IsInRegister == true) ??
                              null!;
        ...
        public async Task<Service> GetServiceDetails(int serviceId)
        {
            ...
            if (flags.HasSupScheme)
            {
                queryWithOptionalIncludes = queryWithOptionalIncludes
                    .Include(p => p.ServiceSupSchemeMapping!.Where(x => x.ServiceSupSchemeCustomDisplayId == null))
        ...
        public async Task<List<Service>> GetPublishedServices()
        {
            return await context.Service.AsNoTracking() //Read only, so no need for tracking query
                ...
                .Include(service =>
                    service.ServiceSupSchemeMapping!.Where(x => x.ServiceSupSchemeCustomDisplayId == null))
                .ThenInclude(ssm => ssm.SupplementaryScheme)
