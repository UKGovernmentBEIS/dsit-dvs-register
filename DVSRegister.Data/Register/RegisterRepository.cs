...
namespace DVSRegister.Data
{
    public class RegisterRepository : IRegisterRepository
    {
        ...
        {
            var trimSearchText = searchText?.Trim();
            ...
            var filteredServicesQuery =
                ...
                where s.IsInRegister
                    ...
                    && (
                    string.IsNullOrEmpty(trimSearchText) || EF.Functions.ILike(s.ServiceName, $"%{trimSearchText}%") || 
                    EF.Functions.ILike(s.Provider.RegisteredName, $"%{trimSearchText}%") || EF.Functions.ILike(s.Provider.TradingName ?? "", $"%{trimSearchText}%"))
                select new
                {
                    Service = s,
        ...
        public async Task<PaginatedResult<Service>> GetServices(List<int> roles, List<int> schemes, List<int> tfVersions, int pageNum, string searchText = "", string sortBy = "")
        {
            ...
            var query = context.Service
                ...
                .Where(s => string.IsNullOrEmpty(searchText) || s.ServiceName.ToLower().Contains(lowerSearchText) || s.Provider.RegisteredName.ToLower().Contains(lowerSearchText))
